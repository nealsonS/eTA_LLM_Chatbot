...
for i in range(len(all_outputs)):
    ...
    print(embedding_str[-40:])
    # Add the text and embedding string to a new DataFrame
    df_new = pd.DataFrame({'text': str(all_materials[i]), 'embedding': embedding_str})
    # Concatenate the new DataFrame with the existing one
    df = pd.concat([df, df_new])

# Save the DataFrame to a CSV file
df.to_csv("course_materials_embeddings_4_13_1.csv", index=False)
